...
const account1 = {
  ...
  movementsDates: [
    ...
    '2025-07-23T17:01:17.194Z',
    '2025-07-25T23:36:17.929Z',
    '2025-07-27T10:51:36.790Z',
  ],
  currency: 'EUR',
  locale: 'pt-PT',
};
...
const account3 = {
  owner: 'Shubh Agrawal',
  movements: [5000, -1337, -3000, 2435, -59],
  interestRate: 2.5,
  pin: 3333,
  movementsDates: [
...
const formatMovementDate = function (movDate) {
  ...
  const calcDaysPassed = (date1, date2) =>
    Math.round(Math.abs(Number(date2) - Number(date1)) / (1000 * 60 * 60 * 24));
...
const formatCurr = function (account, mov) {
...
const displayMovements = function (account, sort = false) {
  ...
  const combinedMovDates = account.movements.map((mov, i) => {
    return {
  ...
  const movs = sort
    ? combinedMovDates.slice().sort((a, b) => a.movement - b.movement)
    : combinedMovDates;
  movs.forEach(function (mov, ind) {
    const type = mov.movement < 0 ? 'withdrawal' : 'deposit';
    const displayDate = formatMovementDate(mov.date);
    const formattedMov = formatCurr(account, mov.movement);
